...
type Filter = 'open' | 'done' | 'skip' | 'all';
...
export default function App() {
  const { store, setTaskStatus, setTaskNote, resetToSeed, importData, exportData } = useStore();
  ...
  function handleReset() {
    if (confirm('Alle Daten zurücksetzen auf Seed? Aktuelle Daten gehen verloren.')) {
      resetToSeed();
      setShowMenu(false);
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 pb-24 pt-4">
      ...
      {progress.percent === 100 && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-emerald-600 text-white px-5 py-2.5 rounded-full text-sm font-bold shadow-lg z-50 animate-bounce">
